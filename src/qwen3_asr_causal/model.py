...
- ``generate_full_hypothesis_rolling`` keeps decoder KV over
  ``[prompt head + cached audio]`` and appends only new audio steps plus the
  prompt tail/draft on each live update.
...
def _register_qwen3_asr_transformers() -> None:
    try:
        ...
        if not hasattr(transformers_generic, "merge_with_config_defaults"):
            transformers_generic.merge_with_config_defaults = lambda fn: fn
    except Exception:
        pass
    ...
    try:
        ...
        if not hasattr(modeling_qwen3_asr, "_qwen3_asr_default_rope_init"):

            def _qwen3_asr_default_rope_init(
                config,
                device=None,
                seq_len=None,
                layer_type=None,
            ...
            modeling_qwen3_asr._qwen3_asr_default_rope_init = (
...
class Qwen3ASRRealtimeQwenDecoderModel(nn.Module):
    ...
    def generate_full_hypothesis_rolling(
        ...
        *,
        ...
        parts: list[torch.Tensor] = []
        reused_audio_steps = 0
        audio_delta_steps = audio_steps
        if cache_valid:
            cache = rolling.cache
            reused_audio_steps = int(rolling.audio_steps)
            delta = frame_hidden[:, rolling.audio_steps :, :].to(
                device=device, dtype=embed_dtype
            )
            audio_delta_steps = int(delta.shape[1])
            if delta.shape[1] > 0:
                parts.append(delta)
        else:
            cache = None
        ...
        stats = {
            ...
            "audio_steps": audio_steps,
            "audio_delta_steps": audio_delta_steps,
            "reused_audio_steps": reused_audio_steps,
            "prompt_head_tokens": len(head_ids),
            "template_tail_tokens": len(tail_ids),
        }
        return tokens, stats
...
class Qwen3ASRRealtimeQwenAudioSurgeryModel(Qwen3ASRRealtimeQwenDecoderModel):
    ...
    def from_qwen_pretrained(
        ...
        *,
        ...
        qwen_model = AutoModel.from_pretrained(qwen_model_id, **load_kwargs)
