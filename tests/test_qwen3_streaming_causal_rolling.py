...
(Part 2). The causal model under test comes from qwen3_asr_causal.
"""
...
from qwen3_asr_causal.causal import (  # noqa: E402
    Qwen3ASRRealtimeQwenAudioCausalModel,
)
from qwen3_asr_causal.model import _split_prompt_template  # noqa: E402
from qwen3_asr_causal.streamer import trim_at_stop  # noqa: E402
...
def test_rolling_steady_state_forwards_far_fewer_positions():
    ...
    assert stats["reused_audio_steps"] == 6
    assert stats["audio_delta_steps"] == 6
    assert stats["audio_steps"] == 12
    assert (
