...
def lookahead_causal_forward(
    tower: nn.Module,
    mels: torch.Tensor,
    *,
    chunk_frames: int = 8,
    left_context_steps: int,
    right_context_steps: int,
    lengths: torch.Tensor | None = None,
    position_offset: int = 0,
) -> torch.Tensor:
    """Differentiable causal tower forward with a bounded future tail.

    This is the latency-oriented training analogue of a mutable-tail streaming
    backend: a step can use a fixed number of future output steps before it is
    frozen. Unlike block-bidirectional training, every step receives the same
    right-context budget, which is the regime we want for latency targets such
    as a 480 ms streaming delay.
    """
    if right_context_steps < 0:
        raise ValueError("right_context_steps must be >= 0")
    hidden = conv_blocks_batched(
        tower, mels, chunk_frames=chunk_frames, position_offset=position_offset
    )
    batch, total_steps, _ = hidden.shape
    positions = torch.arange(total_steps, device=hidden.device)
    allowed = positions[None, :] <= positions[:, None] + int(right_context_steps)
    allowed &= positions[None, :] >= positions[:, None] - left_context_steps + 1
    attn_mask = allowed[None, None, :, :]
    if lengths is not None:
        steps_per_chunk = total_steps * chunk_frames // mels.shape[1]
