...
def replay_text_commit(
    ...
    *,
    ...
    final_flush_append_only: bool = False,
    ...
    if (final_flush_append_only or final_revises_committed) and replayed:
    ...
    *,
    ...
    for row in prediction_rows:
        ...
        replayed, committed_text = replay_text_commit(
            ...
            final_flush_append_only=final_flush_append_only,
            final_revises_committed=final_revises_committed,
        )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--final-flush-append-only",
        action="store_true",
        help=(
            "At EOS, append the final hypothesis only if the already committed "
            "text is still a prefix. This matches the no-past-rewrite final "
            "flush contract."
        ),
    )
            ...
            for min_commit_audio_sec in min_commit_audio_sec_grid:
                results.append(
                    evaluate_policy(
                        ...
                        final_flush_append_only=args.final_flush_append_only,
