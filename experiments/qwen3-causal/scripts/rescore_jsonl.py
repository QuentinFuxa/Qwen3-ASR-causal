...
def rescore_file(jsonl_path: Path, manifest: dict[str, dict]) -> dict:
    ...
    with jsonl_path.open() as f:
        for line in f:
            ...
            hyp = row.get("causal_offline_text") or row.get("final_text") or ""
            real_hyp = row.get("real_streaming_text") or ""
            ...
            item = {
                ...
                "wer_real_streaming_teacher_legacy": (
                    safe_wer(legacy_normalize(teacher_ref), legacy_normalize(real_hyp))
                    if real_hyp
                    else None
                ),
                "wer_real_streaming_teacher_whisper": (
                    safe_wer(whisper_normalize(teacher_ref), whisper_normalize(real_hyp))
                    if real_hyp
                    else None
                ),
                "wer_human_legacy": None,
                "wer_human_whisper": None,
                "wer_real_streaming_human_legacy": None,
                "wer_real_streaming_human_whisper": None,
                "wer_final_original": row.get("wer_final"),
                "wer_causal_offline_original": row.get("wer_causal_offline"),
                "wer_real_streaming_original": row.get("wer_real_streaming"),
            }
            if human_ref:
                ...
                if real_hyp:
                    item["wer_real_streaming_human_legacy"] = safe_wer(
                        legacy_normalize(human_ref),
                        legacy_normalize(real_hyp),
                    )
                    item["wer_real_streaming_human_whisper"] = safe_wer(
                        whisper_normalize(human_ref),
                        whisper_normalize(real_hyp),
                    )
    ...
    return {
        ...
        "wer_causal_offline_original_mean": mean("wer_causal_offline_original"),
        "wer_real_streaming_original_mean": mean("wer_real_streaming_original"),
        "wer_teacher_legacy_mean": mean("wer_teacher_legacy"),
        "wer_teacher_whisper_mean": mean("wer_teacher_whisper"),
        "wer_real_streaming_teacher_legacy_mean": mean("wer_real_streaming_teacher_legacy"),
        "wer_real_streaming_teacher_whisper_mean": mean("wer_real_streaming_teacher_whisper"),
        "wer_human_legacy_mean": mean("wer_human_legacy"),
        "wer_human_whisper_mean": mean("wer_human_whisper"),
        "wer_real_streaming_human_legacy_mean": mean("wer_real_streaming_human_legacy"),
        "wer_real_streaming_human_whisper_mean": mean("wer_real_streaming_human_whisper"),
...
def main() -> None:
    ...
    header = (
        f"{'file':58s} {'n':>3s} {'orig':>6s} {'t-leg':>6s} {'t-whi':>6s} "
        f"{'h-leg':>6s} {'h-whi':>6s} {'rs-hwi':>6s}"
    )
    print(header)
    print("-" * len(header))
    for res in results:
        ...
        print(
            ...
            f"{fmt(res['wer_real_streaming_human_whisper_mean'])}"
        )
