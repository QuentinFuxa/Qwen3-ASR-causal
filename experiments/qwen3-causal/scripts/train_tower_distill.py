...
from qwen3_streaming.tower_distill import (
    ...
    distill_loss,
    lookahead_causal_forward,
    teacher_forward,
)


def resolve_tower_checkpoint(reference: str | Path) -> Path:
    """Resolve a local .pt/.safetensors file, directory, or HF repo id."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=0.0,
        help=(
            "If > 0, train and gate the mutable-tail latency regime: every "
            "audio step receives a bounded lookahead before it is frozen. "
            "For example, 0.48 targets a Voxtral-like 480 ms delay."
        ),
    )
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Load a tower checkpoint before training. Accepts local .pt, local "
            ".safetensors, a directory, or a Hugging Face model repo id."
        ),
    )
...
def build_model(args) -> tuple[Qwen3ASRRealtimeQwenAudioCausalModel, object, object]:
    ...
    config = RealtimeAudioConfig(
        ...
        qwen_audio_mutable_tail_sec=float(args.mutable_tail_sec),
    )
...
def main() -> None:
    ...
    if args.resume_tower is not None:
        resume_metadata = load_tower_checkpoint(student_tower, args.resume_tower)
        print(json.dumps({
    ...
    mutable_tail_steps = int(getattr(model.audio_encoder, "mutable_tail_steps", 0))
    ...
    print(json.dumps({
        ...
        "mutable_tail_sec": float(args.mutable_tail_sec),
        "mutable_tail_steps": mutable_tail_steps,
    }))
    ...
    for step in range(1, args.steps + 1):
        ...
        if mutable_tail_steps > 0:
            student = lookahead_causal_forward(
                student_tower,
                mels,
                left_context_steps=left_context_steps,
                right_context_steps=mutable_tail_steps,
                lengths=lengths,
