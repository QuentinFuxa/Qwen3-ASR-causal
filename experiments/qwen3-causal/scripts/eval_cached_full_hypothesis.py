...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--real-streaming-tail-ms",
        type=float,
        default=250.0,
        help=(
            "Tail hidden from the real-streaming WER at each non-final update. "
            "The default simulates publishing only text older than the last "
            "250 ms, then flushing the final hypothesis without this cut."
        ),
    )
    parser.add_argument(
        "--language",
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Load a causal audio tower checkpoint. Accepts local .pt, local "
            ".safetensors, a directory containing one, or an HF repo id."
        ),
    )
    parser.add_argument(
        "--lora-state-dict",
                    ...
                    items.append(
                        {
                            ...
                            "word_alignments": record.get("word_alignments"),
                        }
                    )
...
def _load_model_and_tokenizer(args: argparse.Namespace, device: torch.device):
    ...
    if args.lora_state_dict is not None:
        lora_payload = torch.load(
            args.lora_state_dict,
            map_location="cpu",
            weights_only=True,
        )
    tower_reference = args.tower_state_dict
    if tower_reference is None and isinstance(lora_payload, dict):
        tower_reference = lora_payload.get("tower_checkpoint")
    if tower_reference is not None:
        metadata = load_tower_checkpoint(
            model.audio_encoder.audio_tower,
            tower_reference,
        )
        print(
            "loaded tower checkpoint: "
            f"{tower_reference} ({json.dumps(metadata, ensure_ascii=False)})"
        )
    if args.lora_state_dict is not None:
        from qwen3_streaming.lora import DECODER_LORA_TARGETS, add_lora_to_linear_modules

        if not isinstance(lora_payload, dict):
            raise RuntimeError(f"invalid LoRA checkpoint: {args.lora_state_dict}")
        payload = lora_payload
        add_lora_to_linear_modules(
            model.text_model,
            target_names=tuple(payload.get("lora_targets") or DECODER_LORA_TARGETS),
...
def _run_one(
    *,
    ...
    causal_offline_text = final.last_hypothesis_text or final.final_text
    real_streaming = simulate_real_streaming_text(
        events,
        final_text=causal_offline_text,
    ...
    return {
        ...
        "reference": reference,
        "final_text": final.final_text,
        "causal_offline_text": causal_offline_text,
        "real_streaming_text": real_streaming["text"],
        ...
        "wer_causal_offline": (
            word_error_rate(reference, causal_offline_text) if reference else None
        ),
        "wer_real_streaming": (
            word_error_rate(reference, real_streaming["text"]) if reference else None
        ),
...
def main() -> None:
    ...
    summary = {
        ...
        "wer_causal_offline_mean": _mean(
            [row.get("wer_causal_offline") for row in ok_rows]
        ),
        "wer_real_streaming_mean": _mean(
            [row.get("wer_real_streaming") for row in ok_rows]
        ),
        ...
        "real_streaming_prefix_mismatch_events_total": sum(
            int((row.get("real_streaming") or {}).get("prefix_mismatch_events", 0))
            for row in ok_rows
        ),
        "real_streaming_blocked_revision_words_total": sum(
            int((row.get("real_streaming") or {}).get("blocked_revision_words", 0))
            for row in ok_rows
        ),
        "real_streaming_final_flush_mismatch_count": sum(
            1
            for row in ok_rows
            if not bool(
                (row.get("real_streaming") or {}).get(
                    "final_flush_prefix_match",
                    True,
                )
            )
        ),
        "real_streaming_coverage_ratio_mean": _mean(
            [
                (row.get("real_streaming") or {}).get(
