...
The validated runtime was promoted into `qwen3_asr_causal/` as a
...
`qwen3_asr_causal/causal.py` (encoder + causal model +
...
## 2026-06-13 - The offset-0 first phase was probably not load-bearing (curriculum retro)

Retrospective on the two-stage training schedule (D1 = fixed 0.96 s blocks at
offset 0; WS2 = D1-continue + mixed 96/192 blocks + position-offset
augmentation). Question: did the offset-0-only first stage earn its keep, or
should offsets have been on from step 0?

Conclusion: **the separate offset-0 stage was most likely NOT necessary; a
single phase with offset + mixed-block augmentation from step 0 should reach
the same place.** The two-stage split is a *discovery artifact*, not a
curriculum decision:

- The phasing exists because the position-extrapolation collapse was found
  *late*. All D1 gates ran on <=16 s chunks, which silently kept positions
  inside the trained 0-200 range; the collapse only surfaced when first probed
  at offset 1300/4000 (see the 2026-06-11 WS1 entry). Offset augmentation was
  then added reactively ("now mandatory in the D1-continue run"), so D1 is just
  "the model from before we knew", not a deliberate warm-up.
- **`--position-offset-prob 0.5` means half of every WS2 step is already at
  offset 0.** So an "offsets from step 0" run is not warm-up-vs-none — it still
  carries a built-in offset-0 signal in 50% of batches, interleaved throughout.
  That self-warms-up the core audio->embedding mapping.
- Offset is data augmentation (teacher fixed at 0, well-defined target from
  step 0) and the distillation loss is a stable teacher-regression, so
  augment-from-start is unlikely to destabilize early convergence.
- Block size generalizes for free (1.92 s eval was 0.265 at WS2 entry, never
  trained on it), so mixed-blocks-from-start is clearly safe.

Steelman for keeping *some* offset-0 warm-up: cheap insurance to let the conv
stem learn the clean mapping before piling on [1,6000] position variance. But
the ~63k D1 steps were vastly oversized for that, and prob=0.5 already supplies
it. Only real risk of offsets-from-start: marginally noisier early convergence
and a small chance the short-form floor lands slightly higher - low-probability,
and unprovable without the counterfactual.

Recommendation for the next run (French / MLS, same recipe): **single phase,
offset (max 6000, prob 0.5) + mixed 96/192 blocks ON from step 0.** Simpler,
and the 50% offset-0 mass self-warms-up. If certainty is wanted, the
counterfactual is ~6 H100 h / ~$20: offsets-from-step-0 vs the two-stage
schedule, compared on the offset-4000 gate trajectory and the short-form floor.
Prior: single-phase reaches long-form-good at least as fast, short-form floor
within noise.

## 2026-06-18 - Voxtral-latency continuation probes

Goal: check whether simply continuing the released causal audio encoder can
move toward a Voxtral-Realtime target: <=480 ms streaming latency and lower
WER. Short answer from the H100 probes: **not with the current embedding-only
tower-distillation recipe.**

Starting point: `qfuxa/qwen3-asr-0.6b-streaming` loaded from `model.safetensors`,
mini LibriSpeech gate `data/ws5_gate_librispeech/gate_clean8_other8.jsonl`
(16 rows), Qwen decoder frozen.

| probe | latency regime | gate start | gate after training | read |
| --- | --- | ---: | ---: | --- |
| mixed 480/960/1920 blocks, LR 3e-6 | append-only block attention | 480: 0.2483 / 960: 0.2287 / 1920: 0.2227 | step 250: 480: 0.2554 / 960: 0.2365 / 1920: 0.2208 | 480 and 960 get worse; stop |
| 480-only, LR 5e-7 | append-only 480 ms block | 480: 0.2483 | step 250: 0.2483, step 500: 0.2483 | flat; stop |
| mutable tail 0.48s, 80 ms tick, LR 1e-6 | bounded lookahead, final-transcript gate | 80: 0.2850 | step 250: 0.2887 | worse than append-only and degrades; stop |

Implementation added for the third probe:
`qwen3_streaming.tower_distill.lookahead_causal_forward()` plus
`train_tower_distill.py --mutable-tail-sec`. The local tests pass, but the
result is negative: the mutable-tail inference regime is mismatched with the
released append-only tower, and the embedding MSE/cosine objective does not
repair it quickly.

Conclusion for the Voxtral target: more H100 time on the same objective is not
the next lever. The remaining plausible routes are decoder-aware:

- train the text decoder/adapters against streaming prefixes (CE/KD on actual
  transcript tokens), not only audio embeddings;
- use a larger, more sensitive latency gate before spending full runs
  (the 16-row WER gate is useful for direction, but too discrete for small
  gains);
- if <=480 ms latency is mandatory, optimize the whole streaming policy around
  80 ms ticks + bounded holdback, then score real streaming WER and emission
  delay together.

Follow-up decoder-aware smoke: add prefix CE on real streaming prefixes, with
the released tower frozen and a small decoder LoRA. Targets are sampled from
forced-aligned words committed before `prefix_end - 250 ms`, so the loss matches
the no-past-rewrite streaming contract more closely than full-transcript CE.

| probe | gate | start | best | later |
| --- | --- | ---: | ---: | ---: |
| prefix CE smoke, 50 steps | 8 rows, 480 ms chunks | 0.1645 | 0.0891 @ step 50 | n/a |
| prefix CE, stopped early | 16 rows, 480 ms chunks | 0.2483 | 0.0920 @ step 100 | 0.1589 @ step 200, 0.1963 @ step 300 |

This is the first positive H100 signal in the Voxtral-latency direction, but it
is only a tiny proxy gate. The next useful check is to load the best LoRA into
the streaming stack and run the real benchmark: LibriSpeech clean/other plus the
MCIF real-streaming WER/latency replay. Do not claim Voxtral-level quality from
this gate alone.

Strict real-streaming follow-up on the same 16-row gate, using
`eval_cached_full_hypothesis.py` with `--chunk-ms 480`,
`--real-streaming-tail-ms 250`, `--normalize-commit-match`, and the released
tower loaded from `qfuxa/qwen3-asr-0.6b-streaming`:

| system | selection gate | WER final/latest | WER real streaming | RTF total | read |
| --- | --- | ---: | ---: | ---: | --- |
| released tower, no LoRA | n/a | 0.2483 | 0.3489 | 1.616 | strict append-only exposes early-prefix errors |
| prefix CE LoRA | final/latest mini-gate | 0.1003 | 0.2113 | 1.984 | big quality gain, still prefix rewrites |
| prefix CE LoRA | real-streaming mini-gate | 0.0925 | 0.2125 | 2.026 | final WER improves; strict stream does not |

The first real-gate run selected step 100:

| step | real-streaming gate WER |
| ---: | ---: |
| 0 | 0.4039 |
| 50 | 0.2213 |
| 100 | 0.1932 |
...
- Prefix CE is a real lever: it roughly halves the strict-streaming mini-gate
  WER vs the released tower.
- It is not enough to beat Voxtral-Realtime yet. The remaining error is now
  dominated by early prefix choices that cannot be repaired later
  (`real_streaming_final_flush_mismatch_count` is 5/16 for both LoRA variants).
  Next run should train the model not only to produce the right prefix, but to
  avoid publishing unstable early words: e.g. an explicit prefix-consistency
  loss across adjacent partial-audio snapshots, or a confidence/wait-token
  policy trained against the append-only scorer.

Implementation note: the strict real-streaming scorer initially over-counted
revisions when the text changed only by case/punctuation (`the` vs `The`).
`normalize_for_match=True` now lowercases in `_common_text_prefix_units()`, and
`test_real_streaming_normalized_match_allows_case_and_punctuation_changes`
guards this.

Second follow-up: the prefix CE labels had a subtle timing bug. The trainer
sampled `prefix_end_sec`, rounded the audio down to a whole 480 ms block, but
kept the target text from the unrounded timestamp. That can teach the decoder
to emit words whose audio has not reached the model yet. Fixed with
`block_aligned_streaming_prefix_target()`, which recomputes the target after
the block-frame floor.

Block-aligned H100 probes, same 16-row 480 ms real-streaming gate:

| probe | best gate | WER final/latest | WER real streaming | RTF total | read |
| --- | ---: | ---: | ---: | ---: | --- |
| p=0.75 prefix CE, 250 ms target holdback, LR 2e-5 | **0.1911 @ step 100** | 0.1197 | **0.1891** | 1.843 | best strict-streaming checkpoint so far |
| p=1.00 prefix CE, 500 ms target holdback, LR 1e-5 | 0.2358 @ step 150 | n/a | n/a | n/a | too conservative; hurts gate |
| p=0.75 prefix CE, 250 ms target holdback, LR 1e-5 | 0.2048 @ step 150 | n/a | n/a | n/a | smoother but worse than LR 2e-5 |

The best block-aligned checkpoint improves strict real streaming from 0.2113 to
0.1891 on the mini-gate. It also reduces blocked live rewrites vs the earlier
LoRA:

| metric | earlier prefix CE | block-aligned prefix CE |
| --- | ---: | ---: |
| WER real streaming | 0.2113 | **0.1891** |
| final-flush mismatch files | 5/16 | **3/16** |
| blocked revision words | 141 | **73** |
| real-streaming coverage | 0.889 | **0.920** |
| first display mean | 0.78 s | 0.78 s |
| first commit mean | 4.80 s | **4.14 s** |

This is progress toward the Voxtral target, but not a win yet. The mini-gate is
still far above Voxtral-quality short-form WER, and the HF eval path is slower
than realtime on this H100 smoke. Next useful move is not a blind longer run;
it is either (1) train the same corrected objective on a larger/diverse gate
and stop on strict-streaming WER, or (2) add an explicit prefix-consistency /
wait-token loss so the model learns when *not* to emit.

Third follow-up: the LibriSpeech mini-gate was not aligned locally and previous
strict-streaming summaries used the uniform text-time fallback. Re-evaluated on
a timestamped WLK gate instead:
`data/wlk_chunks16_teacher_aligned_split_v0/eval_manifest.jsonl`, first 20
English 16 s chunks, all with Qwen3 ForcedAligner word timestamps.

| system | training data | WER final/latest | WER real streaming | RTF total | read |
| --- | --- | ---: | ---: | ---: | --- |
| released tower, no LoRA | n/a | 0.2723 | 0.6975 | 1.204 | true WLK strict stream is much harder |
| block-aligned prefix CE LoRA | mixed WLK+FLEURS+LibriSpeech | 0.2891 | 0.7233 | 1.412 | mini-gate gain does not generalize to WLK |
| block-aligned prefix CE LoRA | WLK-only | 0.2838 | 0.6623 | 1.206 | small domain-matched gain, still far away |

WLK-only training, selected on the first 8 aligned WLK eval chunks:

| step | WLK8 real-streaming gate WER |
| ---: | ---: |
| 0 | 0.6989 |
| 50 | 0.6530 |
| 100 | 0.6382 |
...
This is the most important negative result so far: the LibriSpeech mini-gate was
too narrow. The corrected prefix CE objective can move an aligned WLK gate a bit
(0.6975 -> 0.6623 on WLK20), but it does not solve the real streaming problem.
The failures are still dominated by unstable displayed hypotheses:

| metric on WLK20 | released tower | WLK-only LoRA |
| --- | ---: | ---: |
| prefix mismatch events | 438 | 421 |
| blocked revision words | 2931 | 2649 |
| final-flush mismatch files | 19/20 | 19/20 |
| real-streaming coverage | 0.478 | 0.527 |
| display revision words | 138.5 | 104.6 |

Decision: do not spend more H100 on the current CE-only objective selected on
LibriSpeech. The next meaningful training objective must directly penalize
unstable live text, for example adjacent-prefix consistency / monotonicity, or
a wait/confidence policy that learns to delay words likely to be revised.

Fourth follow-up: added `--prefix-eos-weight` to make prefix CE put extra
weight on EOS for partial-audio examples. This tests the "stop at the stable
prefix" hypothesis directly.

WLK-only, p=0.75 prefix CE, EOS weight 4.0, selected on WLK8 aligned:
...
| system | WER final/latest | WER real streaming | RTF total | blocked revision words | coverage | read |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| WLK-only CE | 0.2838 | **0.6623** | 1.206 | 2649 | **0.527** | better WER/coverage |
| WLK-only CE + EOS x4 | 0.2833 | 0.6773 | 1.309 | **2043** | 0.461 | fewer bad rewrites, but too much silence |

EOS weighting is a useful diagnostic but not enough: it reduces blocked rewrite
mass and final-flush failures (19/20 -> 17/20), but it lowers real-streaming
coverage and worsens WER. The next objective needs a softer emission policy,
not just harder EOS: e.g. predict/wait for only the unstable suffix, or train
adjacent prefixes so the model preserves the previous stable text while adding
only newly safe words.

Fifth follow-up: before more training, replayed the saved WLK8 baseline events
with different no-rewrite publication policies. Added
`--final-flush-append-only` to `tune_stable_commit_from_events.py`: at EOS, the
final hypothesis is appended only when the already committed text is still a
prefix, so it matches the strict no-past-rewrite final flush contract.

Baseline WLK8 (`released tower`, 480 ms chunks, forced-aligner timestamps):

| policy | WER | first commit | coverage | final prefix mismatches | read |
| --- | ---: | ---: | ---: | ---: | --- |
| raw 250 ms tail cut + append-only flush | 0.6647 | n/a | 0.471 | 7/8 | current real-streaming contract |
| default stable commit (6 words, 2 iters, no flush) | 0.6499 | 9.44 s | 0.461 | n/a | safer but still poor |
| tuned stable commit, no EOS flush | 0.5441 | 7.26 s | 0.583 | 3/8 | policy helps but still high WER |
| tuned stable commit + append-only EOS flush | **0.2595** | 11.80 s | 1.000 | 0/8 | recovers final WER by waiting |
| best tuned policy with first commit <=3 s | 0.6771 | 2.58 s | 0.466 | 6/8 | low-latency policy still bad |

Interpretation: the model's final transcript is good enough to make WER
reasonable, and a conservative append-only policy can recover it without past
rewrite. But the cost is latency: ~12 s before the first committed text on 16 s
chunks. For a Voxtral-Realtime target, the missing piece is not just "more
holdback"; the model must produce stable early prefixes. Next training should
use adjacent-prefix consistency / monotonic add-only targets, with the WLK
aligned gate as the checkpoint selector.

Artifacts copied locally:
`/tmp/qwen3_asr_causal_h100_smokes_20260618/`.

Sixth follow-up: trained adjacent-prefix consistency LoRAs on the same WLK-only
aligned manifest. The paired sample takes two nearby block-aligned audio
prefixes from the same utterance; the earlier stable text must be a word prefix
of the later stable text. The loss keeps normal CE on the current target, can
add CE on the earlier prefix, and adds KL(old-prefix logits || new-prefix
logits) over the common stable text. This directly tests whether the decoder can
learn "append new safe words without changing already-safe words".

Implementation notes:

- Added `block_aligned_prefix_pair_targets()` and `consistency_kl_forward()`.
- Added `--prefix-pair-*` knobs to `train_decoder_lora_ce.py`.
- Added `--save-gate-checkpoints`, because the trainer's in-loop gate uses the
  float32 training model while the detailed replay CLI loads inference bf16; for
  decisions, use the detailed replay below.
- All detailed comparisons below use the same L4 CLI replay: WLK8 aligned,
  480 ms chunks, `--qwen-audio-left-context-sec 15`,
  `--qwen-audio-block-bidirectional`, bf16, 250 ms real-streaming tail cut,
  no past rewrite, final flush.

| system | final WER | real-streaming WER | L4 RTF | first display | first commit | coverage | read |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | --- |
| released tower, no LoRA | **0.2625** | 0.6837 | **0.287** | **0.48 s** | 8.10 s | 0.450 | baseline |
| aggressive pair, step 50 | 0.3611 | **0.5914** | 0.336 | 0.78 s | 7.54 s | **0.709** | best real WER, but hurts final |
| soft pair, step 150 | 0.2458 | 0.6577 | 0.438 | 0.90 s | 6.60 s | 0.476 | preserves final, weak real gain |
| mid pair, step 50 | 0.3335 | 0.5918 | 0.366 | 0.78 s | **7.20 s** | 0.681 | similar real gain, slightly better final than aggressive |
| mid pair, step 150 | 0.2673 | 0.6121 | 0.488 | 0.78 s | 7.74 s | 0.614 | best compromise so far |

Pair settings:

- aggressive: prefix CE p=0.75, pair p=0.75, KL weight 0.10, previous-prefix
  CE weight 0.50, selected by the WLK4 smoke gate.
- soft: prefix CE p=0.75, pair p=0.50, KL weight 0.03, previous-prefix CE
  weight 0.25.
- mid: prefix CE p=0.50, pair p=0.75, KL weight 0.10, previous-prefix CE
  weight 0.25; detailed replay scored steps 50/100/150.

Interpretation: adjacent-prefix consistency is a real lever. It lowers
real-streaming WER from 0.684 to about 0.591 on WLK8, and cuts displayed
revision words (baseline 114.6 -> aggressive 70.9 / mid-step50 81.4). But it is
still not enough for the Voxtral-Realtime target. The gain comes partly from
changing what the model emits, and the strongest setting damages the final
transcript. The mid-step150 checkpoint is the current best Pareto point: final
WER is basically baseline (0.267 vs 0.263) while real-streaming WER improves
0.684 -> 0.612, but first committed text is still ~7.7 s and WER is far above
Voxtral-quality.

Next useful move: stop treating "publish/no-publish" as an implicit side effect
of CE/KL. Train or tune an explicit emission policy (wait/confidence/add-only
head) against the real-streaming scorer, with a latency constraint. The current
decoder-only LoRA can make text more stable, but it does not know which suffix
should be withheld.

Artifacts copied locally:
`/tmp/qwen3_asr_causal_training_20260618/wlk8_pair_consistency_l4_summaries.tgz`.

Seventh follow-up: generated full event logs and replayed an oracle append-only
emission policy. The oracle is not deployable: it knows the target transcript
(`target=final` below means the model's own final transcript). It is useful as a
ceiling: if even this cannot commit early, the model hypotheses are too unstable;
if it can, the missing piece is an emission/confidence policy.

Implementation: `scripts/oracle_commit_from_events.py`. It takes the same event
stream as the real scorer, applies the 250 ms tail cut, and commits only the
longest current prefix that is also a prefix of the target. It is strict
append-only: once a surface form has been published, final flush can only append
the remaining suffix, not rewrite punctuation/case/spacing in the past.

WLK8, same bf16 L4 replay as above:

| system | target | final flush | WER | first commit | coverage | committed revisions | read |
| --- | --- | --- | ---: | ---: | ---: | ---: | --- |
| baseline | model final | yes | 0.3726 | **1.44 s** | 1.000 | 0 | oracle can commit early, but surface no-rewrite hurts |
| baseline | model final | no | 0.3726 | **1.44 s** | 0.992 | 0 | almost all final words appeared before EOS |
| mid pair step150 | model final | yes | **0.3017** | 1.56 s | 1.000 | 0 | pair training makes oracle surface more stable |
| mid pair step150 | model final | no | **0.3017** | 1.56 s | 1.000 | 0 | final flush adds nothing; stable text was already there |
| baseline | reference | no | 0.8852 | 1.54 s | 0.155 | 0 | non-deployable reference ceiling: final model errors still dominate |
| mid pair step150 | reference | no | 0.7749 | 1.68 s | 0.260 | 0 | better, but still far from reference prefixes |

Important read: the current model often contains a self-consistent final-prefix
very early (~1.5 s), so the 7-8 s first-commit latency from the hand-written
stable policy is not fundamental. But a strict no-rewrite surface oracle still
scores much worse than the model final (baseline final WER 0.2625 vs oracle
append-only 0.3726; mid final 0.2673 vs oracle append-only 0.3017). That means
the next training target should not only decide *when* to publish; it must also
make early published surface forms match the later final surface. The mid pair
checkpoint is a useful step in that direction.

Next concrete training direction:

- Add a deployable confidence/wait signal to generation (token logit margin,
  wait token, or a tiny commit head over decoder hidden states).
- Supervise it from the oracle labels: commit exactly the longest append-only
  final-stable prefix under the 250 ms tail cut.
- Keep the ASR CE/pair losses, but add a surface-stability loss on committed
  prefixes so punctuation/case/spacing do not get "fixed" later.
- Select checkpoints on a two-axis gate: real-streaming WER and first-commit
  latency (target first commit <=2 s, not the current 7-8 s stable policy).

Artifacts copied locally:
`/tmp/qwen3_asr_causal_training_20260618/oracle_events/wlk8_oracle_commit_events_20260618.tgz`.
