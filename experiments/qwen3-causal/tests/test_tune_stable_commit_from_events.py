...
def test_replay_text_commit_final_flush_appends_only_matching_prefix():
    matching_events = [
        {"audio_sec": 1.0, "hypothesis": "one"},
        {"audio_sec": 2.0, "hypothesis": "one two"},
    ]
    _, committed = tune.replay_text_commit(
        matching_events,
        hold_back_words=0,
        stable_iterations=1,
        final_flush_append_only=True,
    )
    assert committed == "one two"

    revised_events = [
        {"audio_sec": 1.0, "hypothesis": "one wrong"},
        {"audio_sec": 2.0, "hypothesis": "one wrong"},
        {"audio_sec": 3.0, "hypothesis": "one right done"},
    ]
    _, committed = tune.replay_text_commit(
        revised_events,
        hold_back_words=0,
